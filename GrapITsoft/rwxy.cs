...
namespace GrapITsoft
{
    class rwxy :Inherite
    {//人文与法学院
        private string rootHttp = "http://rwxy.hdu.edu.cn";

        public rwxy(int n) : base(n) {
            co_id = "1015";
        }
        ...
        public override bool ReadData()
        {
            ...
            {
                outputData.Trim();
                if (outputData.IndexOf("class=\"about_box\"") >= 0)
                    found = true;
                if (found)
                {
                    if (outputData.IndexOf("<li>") > 0)
                    {
                        ...
                        time[i] = outputData.Substring(outputData.IndexOf("<span class=\"right\">") + 20, 10);
                        title[i] = outputData.Substring(outputData.IndexOf("target=\"_blank\">") + 16);
            ...
            for (int j = 0; j < no; j++)
            {
                ...
                while (!streamRead.EndOfStream)
                {
                    if (outputData.IndexOf("class=\"main\"") >= 0)
                        found = true;
                    ...
                    if (found)
                    {
                        ...
                        outputData = outputData.Replace("&bull;", "");
                        // outputData = outputData.Replace("<br/>", "");
