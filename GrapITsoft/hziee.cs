...
{
    class hziee : Inherite
    {//信息工程
        private string rootHttp = "http://www.hziee.edu.cn";

        public hziee(int n) : base(n) {
            co_id = "1021";
        }
        ...
        public override bool ReadData()
        {
            ...
            {
                outputData.Trim();
                if (outputData.IndexOf("newslist") >= 0)
                    found = true;
                if (found)
                {
            ...
            for (int j = 0; j < no; j++)
            {
                ...
                while (!streamRead.EndOfStream)
                {
                    if (outputData.IndexOf("class=\"centers-right\"") >= 0)
                        found = true;
                    if (outputData.IndexOf("</div>") >= 0)
                        found = false;
                    if (found)
