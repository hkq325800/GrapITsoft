...
{
    class Itsoft : Inherite
    {//软件工程
        private static string rootHttp = "http://itsoft.hdu.edu.cn";

        public Itsoft(int amount) : base(amount) {
            co_id = "1008";
        }
        ...
        public override bool ReadData()
        {
            ...
            while (!streamRead.EndOfStream)
            {
                ...
                int index = outputData.IndexOf("class=\"main\"");
                if (index >= 0)
                    found = true;
                if (found && outputData.IndexOf("<li>") >= 0)
                {
                    ...
                    if (index >= 0)
                    {
                        ...
                        time[i] = outputData.Substring(outputData.IndexOf("class = \"time\">") + 15, 10);
                        i++;
                        if (i > no - 1) break;
                    }
                }
