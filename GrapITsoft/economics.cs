...
{
    class economics : Inherite
    {//经济学院
        private static string rootHttp = "http://economics.hdu.edu.cn/chinese/";
        public economics(int amount) : base(amount) {
            co_id = "1010";
        }
        public override bool ReadData()
        {
            ...
            while (!streamRead.EndOfStream)
            {
                ...
                int index = outputData.IndexOf("td width=\"88%\" height=\"25\" align=\"left\"");
                if (index >= 0)
                    found = true;
                if (found)
                {
                    ...
                    if (index >= 0)
                    {
                        ...
                        time[i] = outputData.Substring(outputData.IndexOf("class=\"zt\">")+11,10);
                        i++;
                        if (i > no - 1) break;
                    }
                }
            ...
            for (int j = 0; j < no; j++)
            {
                ...
                while (!streamRead.EndOfStream)
                {
                    if (outputData.IndexOf("strong class=\"ppx_ht2\">") >= 0)
                    {
                        outputData = outputData.Trim();
                        outputData = outputData.Substring(outputData.IndexOf("ppx_ht2") + 9);
                        title[j] = outputData.Substring(0, outputData.IndexOf("</strong>"));
                        found = true;
                    }
                    if (outputData.IndexOf("作者") > 0)
                        found = false;
                    if (found)
                    {
                        outputData = outputData.Trim();
